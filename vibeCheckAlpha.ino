...
void loop() {
  ...
  //if data from beta garment received then:
  if(ETbeta.receiveData())
  {
    //receive data from beta garment & store it in the updateBeta variable to update BPM 
    betaBPMupdate = betaData.betaBPM;
  }
  ...
  if(counter == 40)//Send data once every 30 seconds
  {
   ...
   //send alphaBPM to beta garment 
   alphaData.alphaBPM = pulseSensor.getBeatsPerMinute(); 
...
////////////////////////////////////////FAST LED//////////////////////////////////////////////
  ...
  int colorValueTemp = map(temp, 18 , 32, 200, 255);// remaping GSR readings to the range of RGB LEDs
...
void heartBeat(){
  int myHeart = betaBPMupdate / 60; //divide the updated BPM by 60
